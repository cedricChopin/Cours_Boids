...
public class BoidsMovement_V2 : MonoBehaviour
{
    private Vector2 velocity;

    public string tagObstacle;
    public float desiredSeparation = 0.5f;
    public float desiredSeparationObstacle = 0.5f;
    ...
    private float squareAvoidanceRadiusObstacle;
    public List<RaycastHit2D> contextHit;
    public List<Transform> context;
    public Vector3[] directions;
    private void Start()
    {
        
        velocity = new Vector2(Random.Range(0, 2), Random.Range(0, 2));
        squareAvoidanceRadius = desiredSeparation * desiredSeparation;
        squareAvoidanceRadiusObstacle = desiredSeparationObstacle * desiredSeparationObstacle;
    ...
    private void Update()
    {
        contextHit = GetNerbyObjects();
        var tmp = contextHit.Select(go => go.transform);
        context = contextHit.Select(go => go.transform).ToList();
        //context = GetNerbyObjects();
        Vector2 sep = Separate(context); // Calcul de la séparation
        Vector2 ali = Align(context); // Calcul de l'alignement
        Vector2 coh = SteeredCohesion(context);// Calcul de la cohésion
        Vector2 stayRadius = StayInRadius();
        Vector2 sepObstacle = SeparateObstacle(contextHit);
    ...
    List<RaycastHit2D> GetNerbyObjects()
    {
        List<RaycastHit2D> context = new List<RaycastHit2D>();
        Collider2D[] contextColliders = Physics2D.OverlapCircleAll(transform.position, neighBordist);
        ...
        for (int angle = -20; angle < 200; angle = angle + increment)
        {
            ...
            hitArray = Physics2D.RaycastAll(transform.position, (point - transform.position).normalized, neighBordist);
    ...
    private Vector2 normalizeDir(Vector2 partialMove, float weight)
    {
        partialMove *= weight;
    ...
    Vector2 SteeredCohesion(List<Transform> context)
    {
        if (context.Count == 0)
            return Vector2.zero;
        ...
        cohesionMove = Vector2.SmoothDamp(transform.up, cohesionMove, ref velocity, 0.5f);
        return cohesionMove;
    }
    /// <summary>
    ...
    Vector2 Align(List<Transform> context)
    {
    ...
    public Vector2 Separate(List<Transform> context)
    {
        if (context.Count == 0)
            return Vector2.zero;
    ...
    public Vector2 SeparateObstacle(List<RaycastHit2D> context)
    {
        if (context.Count == 0)
            return Vector2.zero;
        ...
        Vector2 avoidanceMove = Vector2.zero;
        int nAvoid = 0;
        foreach (RaycastHit2D item in context)
        {
            if (item.transform.tag == tagObstacle)
                {
                 if (Vector2.SqrMagnitude((Vector3)item.point - transform.position) < squareAvoidanceRadiusObstacle)
                    {
                        nAvoid++;
                        avoidanceMove += (Vector2)(transform.position - (Vector3)item.point);
                    }
                }
    ...
    public Vector2 Cohesion(List<Transform> context)
    {
        if (context.Count == 0)
            return Vector2.zero;
